### Euclidea 3 解法  
对应 Apple app store 里的 Euclidea 3 游戏  
  
#### § 1.Alpha 普通⭐:36，隐藏⭐:3，全解完成  
- 教程：直线工具  
解：简单题，略。  
- 教程：圆工具  
解：简单题，略。  
- 教程：点工具  
解：简单题，略。  
- 教程：等边三角形（有隐藏⭐）  
解：简单题，略。  
_注：2V，上下可各画一个三角形得到隐藏星星。_  
- 1.1 作60度角（有隐藏⭐）  
解：简单题，略。  
_注：2V，上下可各画一个角度线得到隐藏星星。_  
- 1.2 作垂直平分线  
解：简单题，略。  
- 教程：垂分线工具  
解：简单题，略。  
- 1.3 找中点  
解：简单题，略。  
- 1.4 正方形内作圆  
解：简单题，略。  
- 1.5 矩形内作菱形（有隐藏⭐）  
解：[3L5E，对矩形对角顶点使用垂分线工具，与矩形边得到交点即为所求菱形的顶点。](solving/Euclidea3/1.5.png)  
_注：2V，左右对称可做两个菱形得到隐藏星星。_  
- 1.6 找圆心  
解：[2L6E，垂分线工具，在圆周上任做2条垂分线，交点即为圆心。](solving/Euclidea3/1.6.1.png)  
解：[5E5E，画圆工具，用三个圆和两条直线得到两条垂分线。](solving/Euclidea3/1.6.2.png)  
- 1.7 圆内作正方形  
解：[6L8E，1.直线连接圆心和给定点得到直径；2.做直径的垂分线。](solving/Euclidea3/1.7.1.png)  
解：[3L3E，1.如图示作圆和辅助线；](solving/Euclidea3/1.7.2.1.png)[7L7E，2.如图示连接交点。](solving/Euclidea3/1.7.2.2.png)  
  
#### § 2.Beta  普通⭐:36，隐藏⭐:2，全解完成  
- 2.1 角平分线  
解：简单题，略。  
- 教程：角平分线工具  
解：略。  
- 2.2 三角形内做菱形  
解：[4L9E，1.作A的角平分线；2.对角平分线作中垂线。](solving/Euclidea3/2.2.png)  
- 2.3 作30度角（有隐藏⭐）  
解：[3L3E，如图示做2个圆。](solving/Euclidea3/2.3.png)  
_注：2V，上下可各画一个角度线得到隐藏星星。_  
- 2.4 2倍角（有隐藏⭐）  
解：[3L3E，如图做两个圆。](solving/Euclidea3/2.4.png)  
_注：2V，上下可各画一个角度线得到隐藏星星。_  
- 2.5 切矩形  
解：[3L3E，穿过矩形中心的线平分矩形面积。](solving/Euclidea3/2.5.png)  
- 2.6 过线外点作垂线  
解：[3L3E，如图做两个圆。](solving/Euclidea3/2.6.1.png)  
解：[2L4E，以给定点为圆心做圆，和直接交点的垂分线必然通过圆心。](solving/Euclidea3/2.6.2.png)  
- 2.7 过线上点作垂线  
解：[1L4E，如图做两个圆。](solving/Euclidea3/2.7.1.png)  
解：[13L3E，1.任意点到O作l圆得到交点A；2.连接圆心和A得到交点B；3.AB连接即为所求垂线。](solving/Euclidea3/2.7.2.png)  
- 教程：垂线工具  
解：简单题，略。  
- 2.8 过圆上点作切线  
解：[2L4E，做半径的垂线。](solving/Euclidea3/2.8.1.png)  
解：[3L3E，1.圆上任意点A为圆心到点O作圆，得到交点B；2.以O为圆心OB为半径作圆；3.连接O与两个圆交点即为所求切线。](solving/Euclidea3/2.8.2.png)  
- 2.9 已知切线作圆  
解：简单题，略。  
- 2.10 菱形内作圆  
解：[4L6E，用直线得到中点，过中点做任一边的垂线得到切点，过中点作圆。](solving/Euclidea3/2.10.png)  
  
#### § 3.Gamma  普通⭐:27，隐藏⭐:3，努力求全解中...  
- 3.1 已知中点求弦  
解：[2L4E，连接两点，做垂线。](solving/Euclidea3/3.1.png)  
- 3.2 已知垂心求三角形  
解：[3L7E，过点分别做2条边的垂线。](solving/Euclidea3/3.2.1.png)  
*解：6E解答还没想出来。*  
- 3.3 已知外心求三角形  
解：[2L2E，如图作圆。](solving/Euclidea3/3.3.png)  
- 3.4 三条线段等长（有隐藏⭐）  
解：[4L6E，作BM的垂分线得到点D，以M为圆心作圆得到点E。](solving/Euclidea3/3.4.1.png)  
_注：[2V，作出左右各一个解得到隐藏星星。](solving/Euclidea3/3.4.2.png)_  
- 3.5 已经切线和圆上一点求圆  
解：[3L7E，作AB和垂分线，过B做垂线，得到交点O，作圆。](solving/Euclidea3/3.5.png)  
*解：6E解答还没想出来。*  
- 教程：移动工具  
解：简单题，略。	  
- 3.6 梯形中线  
解：[3L7E，作两条边的垂分线。](solving/Euclidea3/3.6.png)  
*解：5E解答还没想出来。*  
- 3.7 作45度角（有隐藏⭐）  
解：[2L7E，作点做垂线，然后角平分线工具平分直角。](solving/Euclidea3/3.7.1.png)  
解：[3L5E，1.线上任取一点O，作OA圆；2.过O做垂线，连接交点和A。](solving/Euclidea3/3.7.2.png)  
_注：2V，上下可各画一个角度线得到隐藏星星。_  
- 3.8 菱形（有隐藏⭐）  
*解：5L7E解答还没想出来。*  
_注：[4V，左右上下可各画一个菱形得到隐藏星星。](solving/Euclidea3/3.8.3.png)_  
  
#### § 4.Delta 普通⭐:33，隐藏⭐:4，努力求全解中...  
- 4.1 复制线段（有隐藏⭐）  
解：简单题，略。	  
_注：[2V，画出左侧另一个解得到隐藏星星。](solving/Euclidea3/4.1.png)_  
- 4.2 60度角（有隐藏⭐）  
*解：3L4E解答还没想出来。*  
_注：[2V，左右各画一个角度得到隐藏星星。](solving/Euclidea3/4.2.3.png)_  
- 4.3 等边三角形外接圆  
*解：5L6E解答还没想出来。*  
- 4.4 圆内接等边三角形  
*解：5L7E解答还没想出来。*  
- 教程：交点工具  
解：简单题，略。  
- 4.5 切两个矩形  
解：[5L5E，连接两个矩形中点。](solving/Euclidea3/4.5.png)  
- 4.6 距离√2的点  
*解：3L5E解答还没想出来。*  
- 4.7 距离√3的点  
*解：3L3E解答还没想出来。*  
- 4.8 作15度角（有隐藏⭐）  
*解：3L5E解答还没想出来。*  
_注：2V，上下各做一个15度角得到隐藏星星。_  
- 4.9 已知对边中点求正方形  
*解：6L10E解答还没想出来。*  
- 4.10 切两个矩形（有隐藏⭐）  
*解：7L解答还没想出来。*  
解：[8L10E，如图步骤得到ABCD点并连接。](solving/Euclidea3/4.10.2.png)  
_注：[2V，上下各做一正方形得到隐藏星星。](solving/Euclidea3/4.10.3.png)_  
  
#### § 5.Epsilon 普通⭐:33，隐藏⭐:3，努力求全解中...  
- 5.1 过点做平行线  
解：[2L6E，两次垂线工具可得平行线。](solving/Euclidea3/5.1.1.png)  
*解：4E解答还没想出来。*  
- 教程：平等线工具  
解：简单题，略。  
- 5.2 给定三点做平行四边形（有隐藏⭐）  
解：[4L10E，两次平行线工具可得平行四边形。](solving/Euclidea3/5.2.1.png)  
*解：8E解答还没想出来。*  
_注：[3V，左右下各做一平行四边形得到隐藏星星。](solving/Euclidea3/5.2.3.png)_  
- 5.3 和两个给定点距离相等的线1  
- 5.4 和两个给定点距离相等的线2  
解：[4L10E，两次平行线工具可得平行四边形。](solving/Euclidea3/5.2.1.png)  

- 5.5 梯形的算数平均值  
- 5.6 哈希（有隐藏⭐）  
- 5.7 偏移角  
- 5.8 外接正方形  
- 5.9 正方形内的正方形  
- 5.10 正六边形（有隐藏⭐）  
  
#### § 6.Zeta 普通⭐:30，隐藏⭐:4，努力求全解中...  
- 6.1 中心点镜象线段  
解：[5L5E，轴即为角平分线。](solving/Euclidea3/6.2.png)  
- 6.2 轴镜象线段  
解：[5L5E，轴即为角平分线。](solving/Euclidea3/6.2.png)  
- 6.3 复制线段（有隐藏⭐）  
*解：3L4E解答还没想出来。*  
_注：2V，在给定点两侧各做一条线段得到隐藏星星。_  
- 6.4 给定角平分线  
*解：4L4E解答还没想出来。*  
- 6.5 还原圆  
*解：解答还没想出来。*  
- 教程：半径作圆工具  
解：简单题，略。  
- 6.6 复制线段（有隐藏⭐）  
*解：2L6E解答还没想出来。*  
_注：2V，在给定点两侧各做一条线段得到隐藏星星。_  
- 6.7 已知边长的三角形（有隐藏⭐）  
解：[4L12E，复制半径作圆](solving/Euclidea3/6.7.1.png)  
_注：[4V，如图做4个三角形得到隐藏星星。](solving/Euclidea3/6.7.2.png)_  
- 6.8 斜边和足（有隐藏⭐）  
解：[5L11E，圆周角定理](solving/Euclidea3/6.8.1.png)  
*解：10E解答还没想出来。*  
_注：[4V，如图做4个三角形得到隐藏星星。](solving/Euclidea3/6.8.3.png)_  
- 6.9 平行四边型  
*解：5L8E解答还没想出来。*  
  
#### § 7.Eta 普通⭐:36，隐藏⭐:4，努力求全解中...  
- 7.1 正方形面积之和  
解：[3L11E，复制AB为半径在O点作圆，得到CD；在CD处作垂线得到E。](solving/Euclidea3/7.1.1.png)  
解：[6L6E，1.延长线延得到A点；2.以OA为半径在O处做圆得到BC；3.做圆BO；4做圆CO得到D。](solving/Euclidea3/7.1.2.png)  
- 7.2 环  
*解：解答还没想出来。*  
- 7.3 75度角（有隐藏⭐）  
*解：4L5E解答还没想出来。*  
_注：2V，上下各做一75度角正方形得到隐藏星星。_  
- 7.4 三点等距（有隐藏⭐）    
*解：解答还没想出来。*  
- 7.5 海伦公式  
*解：解答还没想出来。*  
- 7.6 外接圆  
解：[3L7E，两条垂分线可找圆心。](solving/Euclidea3/7.6.png)  
*解：5E解答还没想出来。*  
- 7.7 内接圆  
*解：解答还没想出来。*  
- 7.8 已知中点的线  
解：[3L6E，如图所示求得AB两点连线。](solving/Euclidea3/7.8.1.png)  
*解：5E解答还没想出来。*
- 7.9 等边角  
- 7.10 三分54度角  
- 7.11 外切圆（有隐藏⭐）  
- 7.12 周长平分线（有隐藏⭐）  
解：[4L10E，1.以AD为半径在A作圆；2.以BC为半径在D作圆；3.作CE垂分线，AF即为所求平分线。](solving/Euclidea3/7.12.1.png)  
*解：7E解答还没想出来。*
_注：[3V，如图，作出全部三个解得到隐藏星星。](solving/Euclidea3/7.12.3.png)_  

#### § 8.Theta 普通⭐:33，隐藏⭐:3，努力求全解中...  
- 8.1 最小周长1  
- 8.2 三边平衡  
- 8.3 梯形的调和中项值  
- 8.4 画垂线  
- 8.5 中点  
- 8.6 内角  
- 8.7 正八边形  
- 8.8 三等分梯形对角线  
- 8.9 最小周长2  
- 8.10 线段的调和中项值  
- 8.11 角和中点确定的三角形  
  
#### § 9.Iota 普通⭐:，隐藏⭐:，努力求全解中...  
  
  
#### § 10.Kappa 普通⭐:，隐藏⭐:，努力求全解中...  
  
  
#### § 11.Lambda 普通⭐:，隐藏⭐:，努力求全解中...  
  
  
#### § 12.Mu 普通⭐:，隐藏⭐:，努力求全解中...  
  
  
#### § 13.Nu 普通⭐:，隐藏⭐:，努力求全解中...  
  
  
  

